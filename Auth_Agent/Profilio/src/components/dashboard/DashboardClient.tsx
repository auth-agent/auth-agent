      ...
      if (typeof window !== 'undefined') {
        ...
        if (agentSessionData) {
          try {
            ...
            const { profile: agentProfile, linked_to_user, user_email } = await profileResponse.json()
            ...
            // If agent profile is linked to a user, load the user's profile and context
            if (linked_to_user && agentProfile.user_id) {
              console.log('[Dashboard] Agent profile linked to user, loading user context:', {
                userId: agentProfile.user_id,
                userEmail: user_email
              })

              // When agent profile is linked, the agent profile itself has user_id set
              // Since user_id is unique, this IS the user's profile (or the agent profile became the user's profile)
              // Use the agent profile (which is now linked to the user)
              setProfile(agentProfile)

              // Get social links for this profile (user's social links)
              const { data: userSocialLinks } = await supabase
                .from('social_links')
                .select('*')
                .eq('profile_id', agentProfile.id)
                .order('display_order', { ascending: true })

              setSocialLinks(convertSocialLinks(userSocialLinks || []))

              // Create user object from the linked user
              const linkedUser: User = {
                id: agentProfile.user_id,
                email: user_email || '',
                // Add other user properties if available
              } as User
              setUser(linkedUser)

              console.log('[Dashboard] Loaded user context from linked agent profile:', {
                profileId: agentProfile.id,
                username: agentProfile.username,
                userId: agentProfile.user_id,
                socialLinksCount: userSocialLinks?.length || 0
              })
            } else {
              // Agent profile not linked to user - use agent profile (Fresh Profile scenario)
              console.log('[Dashboard] Agent profile not linked to user, using agent profile')
              ...
              // Get social links for agent profile
