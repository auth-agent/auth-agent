...
const BASE_URL = 'https://api.auth-agent.com';
...
async function main() {
  try {
    ...
    console.log(`AUTH_AGENT_SERVER_URL=https://api.auth-agent.com`);
    console.log(`AUTH_AGENT_CLIENT_ID=${client.client_id}`);
    console.log(`AUTH_AGENT_CLIENT_SECRET=${client.client_secret}`);
    console.log(`NEXT_PUBLIC_AUTH_AGENT_SERVER_URL=https://api.auth-agent.com`);
    ...
    const fs = require('fs');
    const envContent = `AUTH_AGENT_SERVER_URL=https://api.auth-agent.com
